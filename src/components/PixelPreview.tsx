          ...
          <div className="panel flex flex-col">
            ...
            <div className="p-4 flex flex-col gap-4">
              <Control label="Resolution" value={res.label}>
                <select className="select w-full mt-1" value={resIndex} onChange={(e) => setResIndex(Number(e.target.value))}>
                  {RESOLUTIONS.map((r, i) => (
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                <Control label="Text Color">
                  <input type="color" value={textColor} onChange={(e) => setTextColor(e.target.value)} className="w-full h-8 cursor-pointer p-0 border-0" />
                </Control>
                <Control label="Background">
                  <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} className="w-full h-8 cursor-pointer p-0 border-0" />
                </Control>
              </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="panel flex flex-col flex-1">
            <div className="win-title flex justify-between items-center">
              Preview
              <button className="btn btn-ghost py-0 px-2 text-xs" onClick={downloadPng}>Save PNG</button>
            </div>
              ...
              <div className="m-auto flex-shrink-0">
                {pixelMode ? (
                  ...
                  <PixelCanvas
                    ...
                    text={text}
                    family={active.family || active.stack.split(',')[0].replace(/['"]/g, '')}
                    weight={600}
                    small={fontSize}
                    scale={scale}
                    lineH={1.6}
